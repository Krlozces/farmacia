...
public class ProductsDao {
    ...
    public List listProductQuery(String value){
        ...
        String query = "SELECT pro.*, cat.name AS category_name FROM products pro, categories cat WHERE pro.category_id = cat.id";
        String query_search_product = "SELECT pro.*, cat.name AS category_name FROM products pro INNER JOIN categories cat ON pro.category_id = cat.id WHERE pro.name LIKE '%"+value+"%'";        
        try{
            conn = cn.getConnection();
    ...
    public boolean deleteProductQuery(int id){
        ...
        try{
            ...
            JOptionPane.showMessageDialog(null, "No puede eliminar un producto relacionado con otra entidad.");
            return false;
        }
    }
    
    public Products searchProduct(int id){
        String query = "SELECT pro.*, cat.name AS category_name FROM products pro INNER JOIN categories ON pro.category_id = cat.id WHERE pro.id = ?";
        Products product = new Products();
        try{
            conn = cn.getConnection();
            pst = conn.prepareStatement(query);
    ...
    public Products searchCode(int code){
        String query = "SELECT id, name FROM products WHERE code = ?";
        Products product = new Products();
        try{
            conn = cn.getConnection();
            pst = conn.prepareStatement(query);
