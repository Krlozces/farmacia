...
public class SalesDao {
    ...
    public List listAllSalesQuery(){
        ...
        String query = "SELECT s.id AS invoice, c.full_name AS customer, e.full_name AS employee, s.total, s.sale_date FROM sales s INNER JOIN customers c ON s.customer_id = c.id INNER JOIN employees e ON s.employee.id = e.id ORDER BY s.id ASC";
        try{
            conn = cn.getConnection();
            pst = conn.prepareStatement(query);
            rs = pst.executeQuery();
